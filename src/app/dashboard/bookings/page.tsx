...
import {
  LogOut, Calendar, Clock, Video, Trash2, Loader2, AlertCircle, Check, X, ChevronDown, Home
} from 'lucide-react';
    ...
    const loadBookings = async () => {
      try {
        ...
        const bookingsSnap = await getDocs(
          query(collection(db, 'consultation_bookings'), where('clientUid', '==', user.uid))
        );
    ...
    try {
      const response = await fetch('/api/bookings/update-booking', {
        ...
        body: JSON.stringify({
          ...
          consultantUid: booking.consultantEmail, // Using email as fallback
      ...
      if (data.success) {
        ...
        setCancelReason('');
        alert('Booking cancelled successfully. Your consultant will be notified.');
      } else {
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-bold" style={{ color: '#071C3C' }}>My Consultations</h1>
              <p className="text-xs" style={{ color: '#5E6470' }}>View and manage your booked consultations</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <RoleBadge role="user" email={user?.email} size="sm" />
            <Link href="/dashboard" className="p-2 hover:bg-gray-100 rounded-lg transition-colors">
              <Home className="w-5 h-5" style={{ color: '#5E6470' }} />
            </Link>
            <button
                    ...
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
                        <div style={{ color: '#5E6470' }}>
                          <strong>Consultant:</strong> {booking.consultantName}
                        </div>
                ...
                <AnimatePresence>
                  {expandedBookingId === booking.id && (
                    ...
                    >
                      {/* Consultant Info */}
                      <div>
                        <p className="text-sm font-semibold mb-2" style={{ color: '#071C3C' }}>Consultant Details</p>
                        <div className="space-y-1 text-sm" style={{ color: '#5E6470' }}>
                          <p><strong>Name:</strong> {booking.consultantName}</p>
                          <p><strong>Email:</strong> {booking.consultantEmail}</p>
                        </div>
                      </div>
                      ...
                      {booking.description && (
                        <div>
                          <p className="text-sm font-semibold mb-2" style={{ color: '#071C3C' }}>Details</p>
                      ...
                      {booking.meetingLink && (
                        <div>
                          ...
                          <div className="flex items-center gap-2 p-3 rounded-lg" style={{ backgroundColor: '#FFFFFF', border: '1px solid #E5E7EB' }}>
                            <Video className="w-4 h-4" style={{ color: '#10B981' }} />
                            <a href={booking.meetingLink} target="_blank" rel="noopener noreferrer"
                              className="text-sm flex-1 font-semibold break-all" style={{ color: '#10B981' }}>
                              Join Google Meet
                            </a>
                          </div>
                        </div>
                      ...
                      <div className="p-3 rounded-lg" style={{ backgroundColor: '#EFF6FF', borderLeft: '4px solid #3B82F6' }}>
                        <p className="text-xs" style={{ color: '#1E40AF' }}>
                          <strong>📌 Important:</strong> All consultations must take place through this platform. Direct arrangements outside this system are not permitted.
                        </p>
                      </div>
