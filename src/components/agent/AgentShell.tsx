...
const NAV_FA: Record<string, string> = {
  'Dashboard':        'داشبورد',
  'Enquiries':        'استعلام‌ها',
  'Clients & Cases':  'موکلین و پرونده‌ها',
  'Document Review':  'بررسی اسناد',
  'Reports':          'گزارش‌ها',
};
...
export default function AgentShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen" style={{ backgroundColor: '#F8F9FA' }}>
      {/* ── Sidebar ── */}
      <aside className="w-56 flex-shrink-0 flex flex-col" style={{ backgroundColor: '#071C3C', borderRight: '1px solid #0B234A' }}>
        {/* Logo */}
          ...
          {NAV_EN.map(item => {
            ...
            return (
              <Link
                ...
                style={{
                  backgroundColor: active ? '#0B234A' : 'transparent',
                  color: active ? '#C9A35A' : '#94A3B8',
                }}
              >
        ...
        <div className="px-3 py-4 border-t space-y-1" style={{ borderColor: '#0B234A' }}>
          <Link href="/" className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-xs font-medium" style={{ color: '#64748B' }}>
